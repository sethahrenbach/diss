...
\begin{figure}[H]
	...
	\path (w) edge node[bend left] {$R$} (w)
%	\path (w) edge [out=170,in=180,looseness=8] node[right] {$R$} (w)
	(v) edge node[below] {} (w);
	\end{tikzpicture}
	\end{center}
...
According to possible world semantics, $\Box \varphi$ is true at a world, written $w \models \Box \varphi$ for world $w$, if and only if for all worlds $v$ such that $R(w,v)$ ($v$ is accessible to $w$ by the $R$ relation), $v \models \varphi$. This says a formula is necessarily true at a world if and only if it is true at all worlds accessible by that world. If a formula is true at all worlds in a model, then it is valid for that model, written $\mathcal{M} \models \varphi$. Clearly, if a formula is valid for a model, then it is necessarily true for each world in that model: $\mathcal{M} \models \varphi$ implies $w \models \Box \varphi$ for all worlds $w\in W$. In our model, $p$ is not valid: it is false at world $w$. So a
