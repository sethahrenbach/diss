...
This chapter describes the context in which my research makes advances. Section~\ref{fm} describes the state of formal methods. Section~\ref{logic_foundation} introduces the logic to be used in exploiting this opportunity, a variation of dynamic epistemic logic modified for reasoning about pilots. Section 2.3 describes related work involving logical approaches to agents in system verification. Section 2.4 describes the Coq Proof Assistant, a tool for constructing mechanically checked mathematical proofs, to be used in this research.
...
We are concerned with an additional element: the \emph{safety} status of an action, and an agent's knowledge and belief about that. To capture this, we extend DEL and call the new logic Dynamic Agent Safety Logic (DASL).
