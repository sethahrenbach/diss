...
The standard translation is fed into the Sahlqvist-van Benthem algorithm for producing a corresponding frame condition. The algorithm works by abstracting each predicate and yielding a (monadic) second-order formula, and then instantiating each predicate as a lambda function. 

Blackburn \etal\ refer to the translation into monadic second-order\footnote{monadic second order logic is restricted to predicates over sets.} logic as a preprocessing step, in addition to bookkeeping like ensuring that $x$ is not bound, and that no two quantifiers bind the same variable.
...
\begin{eqnarray}
(\exists x, \gamma(x) \mathtt{\ and\ }\psi )\Longleftrightarrow \exists x, (\gamma(x) \mathtt{\ and\ }\psi)\\
(\exists x, \gamma(x) \Longrightarrow \psi) \Longleftrightarrow \forall x, (\gamma(x) \Longrightarrow \psi),
\end{eqnarray}
where $\gamma(x)$ is a first order sentence with one free variable $x$.

\emph{Step 2.} Instantiate predicates $P(x)$ to functions of the form $\lambda u.u\neq u$.
