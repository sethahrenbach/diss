...
\begin{definition}[Standard Translation]. For binary modal operators $\Bels{i}$ and $\Kns{i}$, we define the function $ST$ on a given variable $x$ and proposition letter $p$:
	\begin{align*}
	ST(x, p) &= P(x) \\
	ST(x, \bot) &= (x \neq x) \\
	ST(x, \tlnot\varphi) &=\ \sim(ST(x, \varphi)) \\
	ST(x, \varphi \tlor \psi) &= ST(x, \varphi) \mathtt{\ or\ } ST(x, \psi)\\
...
Sahlqvist formulas are not necessarily uniform in their propositions, but a similar technique is used to generalize translated predicates into a monadic second order formula, and instantiate them to minimal valuations. The monadic second order standard translation is fed into the Sahlqvist-van Benthem algorithm for producing a corresponding frame condition.

The Sahlqvist-van Benthem algorithm is described in the following way.

\emph{Step 1.} Apply the following equivalences, from left to right, (in this order):
\begin{eqnarray}
(\exists x, \varPhi(x) \mathtt{\ and\ }\psi )\Longleftrightarrow \exists x, (\varPhi(x) \mathtt{\ and\ }\psi)\\
...
\emph{Step 2.} Construct minimal valuations for each proposition letter instance. Proposition letter instances are converted to predicates over variables. Our intuitive interpretation is that the variables are worlds from the frame. An appropriate lambda function can be created for each predicate such that the smallest frame possible validates it. For a proposition letter $p$ appearing once in a formula, either as an atom or behind a diamond (\emph{e.g. $\BPoss{i}p$}), $\lambda u.u=x$ is created, assuming $x$ is the appropriate variable appearing in second order translation $P(x)$. 

If $p$ appears twice, either as an atom or behind a diamond, each instance appears in the translated sentence in some form like $\exists x_1(\Rel{b}^i(w,x_1) \mathtt{\ and\ } P(x_1))$ (for diamonds) $... \mathtt{\ and\ }P(x_2)$. The lambda function to serve as $P$'s instance is $\lambda u.(u=x_1 \text{\ or\ }u=x_2)$. For more than two appearances as atoms or behind diamonds, this pattern is repeated.
...
\emph{Step 3.} Perform the instantiations. This step substitutes the predicate variables for the minimal valuations constructed in \emph{Step 2}. For example, if $P(x)$ appears in the second order formula, with the minimal valuation substitution $\lambda u.u=x$, the instantiated predicate instance yields the sentence $(\lambda u.u=x_1)(x)$, which beta reduces to $x=x_1$. If the proposition letter instance appears behind a box in the modal formula, \emph{e.g.} $\Bels{i}p$, the second order formula is $\forall y(\Rel{b}^i(x_1, y) \mathtt{\ implies\ }P(y))$ with the lambda function $\lambda u.(\Rel{b}^i(x_1, u))$, which yields the formula  $\forall y(\Rel{b}^i(x_1, y) \mathtt{\ implies\ }\lambda u.(\Rel{b}^i(y, x_1)))(y)$. This beta reduces to $\forall y(\Rel{b}^i(x_1, y) \mathtt{\ implies\ }\Rel{b}^i(x_1, y))$, which is equivalent to $\forall y, \Rel{b}^i(x_1, y)$. For examples, see Blackburn \etal\cite{modal} chapter 3.
...
where \emph{hold} refers to frame validity and metalanguage semantics, respectively.
\begin{proof}
	$\\$
	$\Kns{i}p \iimplies p$. Applying the above translation rules, where the normal equivalence of $\sim q \mathtt{\ or\ }r\equiv q \Longrightarrow r$, yields $(\forall y,\ \Rel{k}^i(x,y) \Longrightarrow P(y)) \Longrightarrow P(x)$.
\end{proof}
