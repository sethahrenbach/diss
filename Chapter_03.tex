...
$\mathbf{Proof\ Sketch}.$ Completeness states that if a formula $\varphi$ is valid, then it is deducible. The sketch for this proceeds proceeds by contraposition. \\
$\mathbf{Proof\ Goal}.$ For every formula $\varphi$ in the language $\mathcal{L}_{DASL}$, $\not\vdash\varphi \timplies \not\models\varphi.$\\
...
$\mathbf{Lindenbaum}$: Every consistent set of formulas is a subset of a maximal consistent set of formulas.\\
$\mathbf{Properties}$: If $\Gamma$ and $\Delta$ are maximal consistent sets, then:
\begin{itemize}
...
\end{itemize}
$\mathbf{Truth}$: For every $\varphi \in \mathcal{L}_{DASL}$, and every maximal consistent set $\Gamma$:\\

\begin{eqnarray*}
	\varphi \in \Gamma \iiff (M^C,\Gamma)\models\varphi
\end{eqnarray*}
$\mathbf{Canonicity}$: The canonical model satisfies the above frame conditions.\\
