...
\begin {tikzpicture}[auto ,node distance =7 cm and 7cm ,on grid ,
	...
	state/.style ={ rectangle ,top color =white , bottom color = white ,
		draw, text=black}]
	\node[state] (A) {\begin{tabular}{l} $pre$: \emph{heads} \\ $post$: $\emptyset$
	\end{tabular}};
	\node[state] (B) [right =of A] {\begin{tabular}{l} $pre$: \emph{tails} \\ $post$: $\emptyset$
	\end{tabular}};
	\path[]
...
\begin{figure}[H]
	\begin{center}
		...
		\node[state] (A) 
			{\begin{tabular}{l} 
				$pre$: $\top$ \\ $post$: \{$\top \rightarrow$ \emph{heads}\}
			\end{tabular}};
		\node[state] (B) [right =of A] 
			{\begin{tabular}{l} 
				$pre$: $\top$ \\ $post$: \{$\top \rightarrow$ \emph{tails}\}
			\end{tabular}};
		\node[state] (C) [below right =of A]
			{\begin{tabular}{l}
				$pre$: \emph{tails} \\ $post$: $\emptyset$
			\end{tabular}};
		\node[state] (D) [below =of A]
		{\begin{tabular}{l}
				$pre$: \emph{heads} \\ $post$: $\emptyset$
			\end{tabular}};
		\path[]
...
In Figure~\ref{fig: secret_peek}, Alice knows the state of the coin, Bob has the false impression that Alice is, like him, totally ignorant about the coin's state, and Alice knows this about him.
