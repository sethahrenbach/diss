...
In constructing our static base, we must identify the desirable features to be achieved. In laying out these desirable features, we motivate our model that combines knowledge and belief. 

First, the model should be well-balanced in the trade off of normativity \emph{vs.} realism so that it can be used to analyze real-world cases as well as idealized formal cases.
...
Third, the model should be a sound and complete logical system to serve as a static foundation for an extension that treats actions. Just because attention must be paid to the philosophical foundations and motivations for a formal system does not mean that the formal system is off the hook for the standard desiderata of a logic. The logic should also seek to balance expressive power and complexity, which are always in competition with each other.
