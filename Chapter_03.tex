...
\begin{enumerate}
	...
	\item The axioms of Knowledge are nonstandard and seem contrived
	\item The axioms relating Belief and Knowledge are nonstandard and the converse of Hintikka's axiom relating the two.
\end{enumerate}
We will now address each concern and defend the axioms presented above.
...
\begin{enumerate}
	\item A model treating a pilot's beliefs is simpler than a model treating a pilot's beliefs about her knowledge.
	\item Models treating intelligence (both artificial and natural) show that beliefs alone, with knowledge, are sufficient.
	\item Knowledge in the cockpit is so rare that its inclusion in the model is not necessary.
	\item Implicitly, if model A is simpler than model B, and A is sufficient for the phenomenon under investigation, and B contains elements not necessary for modeling the phenomenon, then model A is superior to model B.
	\item Therefore, the BDI model is superior to the DASL model.
\end{enumerate}

\subsubsection*{Knowledge Axioms}
