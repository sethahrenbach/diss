...
This section establishes the soundness and completeness of the logic.  We present mechanized formalizations of each proof in the Coq Proof Assistant. The proof of soundness shows that each axiom corresponds to a frame relation definiing the models. The proof of soundness shows that each axiom schema is a Sahlqvist formula, and therefore forms a complete logic built by adding them to the axiom K of the minimal modal logic.
...
\subsection{Comleteness}~\label{sec:completeness}
	...
	Fixpoint negative_formula (phi : formula) : Prop :=
		match phi with
			...
			| FOr phi1 phi2 => (negative_formula phi1) 
				\/ (negative_formula phi2)
			| FImp phi1 phi2 => (not (negative_formula phi1)
				 \/ negative_formula phi2)
...
Fixpoint s_a_component (phi : formula) : Prop :=
		match phi with
			...
			| FNeg phi' => match phi' with
				| FProp p => True
				| FAnd p1 p2 => s_a_component p1 /\ s_a_component p2
				| FOr p1 p2 => s_a_component p1 /\ s_a_component p2
				| FImp p1 p2 => False
				| FNeg p' => s_a_component p'
				| FK a p' => s_a_component p'
				| FB a p' => s_a_component p'
				end
			| FK a phi' => boxed_formula phi'
			| FB a phi' => boxed_formula phi'
		end.
