...
The logic for reasoning about information flow in knowledge games is called Dynamic Epistemic Logic (DEL). As its name suggests, it combines elements of epistemic logic and dynamic logic. Epistemic logic is the static logic for reasoning about knowledge, and dynamic logic is used to reason about actions. In dynamic logic semantics, nodes are states of the system (or of the world), and relations on nodes are transitions via programs or actions from node to node. If we think of each node in dynamic logic as being a distinct model of static epistemic logic, then actions become relations on models, representing transitions from one multi-agent epistemic model to another. For example, if we have a static epistemic model $M1$ representing the knowledge states of agents $1$ and $2$ at a moment, then the action $``p?"$ is a relation between $M1$ and $M2$, where $M2$ is a new static epistemic model of $1$'s and $2$'s knowledge after the question is asked. All of this is captured by DEL.
...
\begin{align*}
	\Poss{i}&\equiv \tlnot \Kns{i}\tlnot \\
	\BPoss{i}&\equiv \tlnot \Bels{i}\tlnot \\
	\PalPos{A,\laa}&\equiv\tlnot \Pal{A,\laa}\tlnot \\
	\SPalPos{A,\laa}&\equiv\tlnot\SPal{A,\laa}\tlnot 			
\end{align*} 
...
As far as we can tell, this axiom scheme is novel, and disagrees with other attempts to model the interaction of knowledge and belief. The primary point of departure is that the knowledge operator is no longer an \SFive\ operator, but merely a T operator, defined by the axiom scheme $\Kns{i}\varphi \iimplies \varphi$. Thus, it is severely weakened as a modal operator, but the corollary to this fact is that knowledge no longer has the imposing necessary conditions of positive and negative introspection. 

This section presents the desirable features that a static logic should have, and along the way argues against \SFive's appropriateness as an epistemic base. Before beginning, we briefly establish a consistent informal interpretation of knowledge and belief.
...
%We do not take this path in interpreting the knowledge operator. Instead, depart from the majority of formal epistemologists and interpret the knowledge operator as what is actively known by an agent at a moment.
...
Epistemologists primarily seek a theory of justification that coheres with our considered intuitions about cases, and a theory of knowledge that can solve the problem of skepticism. A system of formal epistemology should respect these questions by coherently addressing them. We consider it required for a system of epistemic logic to be evaluated as a theory of epistemology. The epistemic logic of \SFive\ with common knowledge does not strive for this. We briefly recount some broad positions of interest from epistemology that our model should take a considered position on.
...
%Knowledge is weaker here than in most epistemic logics~\cite{FHMV,DEL,Hintikka}. Knowledge is almost universally formalized as an \SFive\ modal operator, including axiom schema for positive and negative introspection. Rushby suspects that because the analyses of aviation mishaps lays the blame on a failure of negative introspection in~\cite{AhrenbachGoodloe}, the decision to exclude the negative introspection axiom from the schema appears contrived. Rushby proves that to include the negative introspection axiom would cause a collapse of knowledge and belief into equivalence, due to the axiom \emph{Evidential Restraint}: $\Bels{i}\varphi \iimplies \Bels{i}\Kns{i}\varphi$, which to our knowledge was first proved by Lenzen~\cite{Lenzen}. \emph{Evidential Restraint}, additionally, is the converse of Hintikka's choice for relating the two: $\Bels{i}\Kns{i}\varphi \iimplies \Bels{i}\varphi$. Axiom \emph{Evidential Restraint} will be defended below. Rushby's concern with the knowledge axioms, or lack thereof, can be summarized as follows.
%
%\begin{enumerate}
%	\item Negative introspection is a standard axiom for epistemic logic.
%	\item \emph{Evidential Restraint} is non-standard for multi-modal epistemic-doxastic logics.
%	\item Including negative introspection and excluding \emph{Evidential Restraint} preserves an analysis based on false beliefs that is analytically sufficient.
%	\item Standard axioms are preferable to non-standard axioms when standard axioms are analytically sufficient.
%	\item Therefore, including negative introspection and excluding \emph{Evidential Restraint} is preferable to excluding and including respectively.
%\end{enumerate}
...
\section{Interpreting $\Kns{i}$ and $\Bels{i}$}
...
%Later, suppose she misremembers the event, and believes that she saw the coin land \emph{tails}. In that case, doxastically possible \emph{tails}-worlds proliferate. However, our logic requires that doxastic relations are subsets of epistemic relations. Therefore, for this misremembered doxastic relation, there must be some objectively available evidence supporting \emph{tails}. This seems to be a problem. The problem is resolved if we recognize that the agent is not imagining from nothingness to perception of a coin landing tails, and that there have been some times in her life when she observed light from \emph{tails} coins, and she is simply confusing the instances in her memory.

In the case of false beliefs, suppose Bob is an early bird in 1948 America, and he reads the headlines that \emph{DEWEY DEFEATS TRUMAN}. Here we have objectively available evidence, a source of information in the actual worlds that supports a proposition. One of the propositions supported is that Dewey defeated Truman in the 1948 United States Presidential election. This proposition happens to be false, but it nonetheless has objectively available evidence supporting it. Thus, objectively available evidence does not necessarily guarantee truth for the propositions it supports. It merely lends support to them. When Bob reads the newspaper, he may or may not form the belief that Dewey won the election, depending on the other evidence he has perceived.

%is also subjectively available evidence. But the objectively available evidence is still the light that reflected off the coin's surface, which was in a \emph{heads} up orientation at the time.


%At doxastically possible worlds, further possible worlds may branch off via the epistemic relation, but they are no longer generated by objectively available evidence. Instead, they are generated by subjective evidence of objective evidence. Consider the early bird Bob case. Suppose he forms the belief that Dewey defeated Truman, and he regards this belief as knowledge: after all, it's in the papers. At all doxastically possible worlds, any reference to objective evidence, like the newspaper headline, references to polling data from the article, etc


%When the coin lands \emph{heads} up, all epistemically possible worlds become constrained to those in which the proposition ``the coin landed \emph{heads} up" is true. When $i$ sees it happen, doxastically possible worlds exist in which the propositions ``the coin landed \emph{heads} up" and ``The evidence tells me that the coin landed \emph{heads} up" and ``I know that the coin landed \emph{heads} up" are true, because she is perceiving the objectively available evidence. Her perception of it makes it subjective, but the perception is that it is objective, which is true of the information flowing from the event, but not of the information flowing from her perception of it.
 ...
 Thus, we have provided an interpretation of the possibility relations underlying knowledge and belief, which is not often done in the literature. We now present our theory's notion of justification and its response to skepticism.
 
 \subsection{Justification}\label{sec:just}
 Justification in our logic is not represented explicitly with a justification operator, as in Artemov and Nogina \cite{ArtemovNogina}. There, they formalize justification and knowledge with a formula like $t:\varphi$, where $t$ is a justification (a proof-like object) for $\varphi$ polynomial in its length. They adopt $\mathcal{S}4$, a modal logic defined by the axiom schemas $\Kns{i}\varphi \iimplies \Kns{i}\Kns{i}\varphi$ plus $\Kns{i}\varphi \iimplies \varphi$, as the static base epistemic logic, and interpret knowledge in the usual implicit or subjunctive way. They include a connective axiom schema $t:\varphi \iimplies \Kns{}\varphi$, read ``if $\varphi$ is justified then $\varphi$ is knowable". We seek a weaker notion of justification here, under the notion that false propositions can nonetheless be justified by evidence. Rather than representing justification explicitly in the object language, we bury it in the underlying model theory, relating evidence to the possibility relations over worlds. We capture the notion of justification in the object language with the axiom schema \emph{Evidential Restraint}.
...
The key to achieving this is through the composition of the doxastic relation with the epistemic relation. From a first person perspective, our model does collapse belief and knowledge together. That means that an agent cannot distinguish her mere beliefs from her genuine knowledge. Let $\Rel{k/b}^i(w)$ denote the extension of the epistemic/doxastic relation of $w$.\footnote{The \emph{extension} of a relation $R$ is a set of pairs $(w,v)\in R$, and the \emph{extension} of a relation $R$ of $w$ is a set $\{v|(w,v)\in R\}$} We also define the following mathematical notion on the relations.
...
It is important that we maintain the distinction between knowledge and belief if we aim to model humans. To show that our logic maintains this distinction, we present the following counterexample.
...
\begin{theorem}[Soundness]
	Dynamic Agent Safety Logic's static base is sound for Kripke structures with\\ (1) reflexive $\Rel{k}^i$ relations,\\ (2) serial, Euclidean $\Rel{b}^i$ relations, \\(3) which are partially ordered $(\Rel{k}^i \circ \Rel{b}^i) \subseteq \Rel{b}^i$, and $\Rel{b}^i \subseteq \Rel{k}^i$.\footnote{We ignore the following dynamic frame conditions for now:  $\llbracket (A,\laa)_i \rrbracket^S \subseteq \llbracket (A,\laa)_i \rrbracket$ and 
	$\llbracket (A,\laa)_i \rrbracket \subseteq (\llbracket (A,\laa)_i \rrbracket^S \circ \Rel{b}^i)$, although they are also \DASL\ frame conditions.}
\end{theorem}
...
\begin{theorem}[Sahlqvist Correspondence Theorem\cite{modal}\footnote{Theorem 3.54 of Chapter 3 in Blackburn \etal.}]
...
The SComT is:
\begin{theorem}[Sahlqvist Completeness Theorem\cite{modal}\footnote{Theorem 4.42 of Chapter 4 in Blackburn \etal.}]
...
\begin{definition}[Standard Translation] For unary modal operators $\Bels{i}$ and $\Kns{i}$, we define the function $ST$ on a given variable $x$ and proposition letter $p$:
	\begin{align*}
	ST(x, p) &= P(x) \\
	ST(x, \bot) &= (x \neq x) \\
...
%
%Sahlqvist formulas are not necessarily uniform in their propositions, but a similar technique is used to generalize translated predicates into a monadic second order formula, and instantiate them to minimal valuations. 
...
\begin{lemma}[Schema to First Order Frame Condition Correspondence]\label{schema_to_frame}
...
\begin{proof} We must show a translation from the following axiom schemas to the appropriate first order frame conditions:
	...
	$\forall y, (\Rel{k}^i(x,y)\Longrightarrow \lambda u.(\Rel{k}^i(x,u))(y))\Longrightarrow \lambda u.(\Rel{k}^i(x,u))(x)$, which beta reduces to:\\
	$\forall y, (\Rel{k}^i(x,y)\Longrightarrow\Rel{k}^i(x,y))\Longrightarrow \Rel{k}^i(x,x)$, which is equivalent to $\Rel{k}^i(x,x))$.
	
	$\Bels{i}\varphi \iimplies \Bels{i}\Kns{i}\varphi$. We skip ahead to this more interesting example.
	\begin{align*}
		&\rightsquigarrow \forall P,(\forall y,\Rel{b}^i(x,y)\Longrightarrow P(y)) \Longrightarrow (\forall z,z' (\Rel{b}^k(x,z)\cdot \Rel{k}^i(z,z')) \Longrightarrow P(z')) \\
		&\rightsquigarrow \forall y,(\Rel{b}^i(x,y)\Longrightarrow \lambda u. (\Rel{b}^i(x,u))(y)) \\ &\text{\ \ \ \ }\Longrightarrow \forall z,z',(\Rel{b}^i(x,z) \cdot \Rel{k}^i(z,z')\Longrightarrow \lambda u.(\Rel{b}^i(x,u))(z'))	\\
		&\rightsquigarrow \forall y, (\Rel{b}^i(x,y)
		\Longrightarrow \Rel{b}^i(x,y)) \\ &\text{\ \ \ \ }\Longrightarrow \forall z,z',(\Rel{b}^i(x,z) \cdot \Rel{k}^i(z,z')\Longrightarrow \Rel{b}^i(x,z')\\
		&\equiv  \forall z,z',(\Rel{b}^i(x,z) \cdot \Rel{k}^i(z,z')\Longrightarrow \Rel{b}^i(x,z')
	\end{align*}	
	...
	$\forall z, (\Rel{k}^i \circ \Rel{b}^k )(x,z) \Longrightarrow \Rel{b}^i(x,z)$, which is the subset relation of the frame condition.
	...
	\begin{align*}
		...
		&\rightsquigarrow \forall y, (\Rel{b}^i(x,y)\Longrightarrow\Rel{b}^i(x,y)) \Longrightarrow \exists z, (\Rel{b}^i(x,z))\\
		&\equiv \exists z, \Rel{b}^i(x,z)	
	\end{align*}
	which is equivalent to frame condition for seriality.
	...
	\begin{align*}
		...
		&\rightsquigarrow \forall y, (\Rel{k}^i (x,y) \Longrightarrow \lambda u.(\Rel{k}^i(x,u))(y))\Longrightarrow \forall z, (\Rel{b}^i (x,z) \Longrightarrow \lambda u. (\Rel{k}^i(x, u))(z))\\
		&\rightsquigarrow \forall y, (\Rel{k}^i(x,y) \Longrightarrow \Rel{k}^i(x,y)) \Longrightarrow \forall z, (\Rel{b}^i(x,z) \Longrightarrow \Rel{k}^i(x,z))\\
		&\equiv \forall z,  \Rel{b}^i(x,z)\Longrightarrow\Rel{k}^i(x,z))\\
	\end{align*}
...
\begin{theorem}[\DASL\ complete on \DASL\ frames]\label{completeness}
...
\end{proof}
%must appeal to a theorem that we have not mechanized, presented in Blackburn \etal\cite{modal}. In it, they present the theorem,
%
...
%\end{theorem}
%\begin{proof}
%	The former claim follows from the latter. See \cite{modal} chapter 3 for details on the algorithm.
%\end{proof}

%Since $c_\chi(x)$ defines the frame, $\chi$ is valid for that class of frames. So, by our proof of soundess where we show that the corresponding modal formulas are sound for their corresponding first order frame conditions, we can infer that these are the frame conditions for which the \DASL\ axioms are complete via the Sahlqvist theorem (\ref{thm:sahlqvist}). We leave the mechanization of these steps to future work. However, 

%We mechanize the preservation of completeness from $\mathtt{schema}$s to $\mathtt{prop}$s:
%
%\begin{tcolorbox}
%	\begin{lstlisting}[language=Coq]
...
To finish off our defense of \DASL, we must show that the dynamic extension with these non-reductive axiom schemas is sound and complete. Recall the semantics for the dynamic operators:
\begin{table}[H]
\begin{center}
\begin{tabular}{|c|}
\hline
...
\hline 
\end{tabular}
\caption{Dynamic operator semantics}
\end{center}
\end{table}
Additionally, the dynamic model relations are defined as:
\begin{table}[H]
\begin{center}
\begin{tabular}{|c|}
\hline
...
\begin{theorem}[Dynamic Extension Soundness]
	Dynamic Agent Safety Logic's dynamic extension is sound for Kripke structures with \\
	(1) $\llbracket (A,\laa)_i \rrbracket^S \subseteq \llbracket (A,\laa)_i \rrbracket$\\
	(2) $\llbracket (A,\laa)_i \rrbracket \subseteq (\llbracket (A,\laa)_i \rrbracket^S \circ \Rel{b}^i)$
\end{theorem}
\begin{proof}
For soundness with respect to $(1)$, we use \emph{Inevitability}:\\
$\Pal{A,\laa}\varphi \iimplies \SPal{A,\laa}\varphi$. Note that condition $(1)$ is equivalent to $\forall z, (x \llbracket (A,\laa)_i \rrbracket^S z \Longrightarrow x \llbracket (A,\laa)_i \rrbracket z)$.

Assume $M,w \models \Pal{A,\laa}\varphi$. By the semantics of $\Pal{A,\laa}$, for all $M', w'$, if \\$(M,w)\llbracket (A,\laa)_i \rrbracket (M',w')$, then $M',w'\models \varphi$. By condition $(1)$, $(M,w)\llbracket (A,\laa)_i \rrbracket^S (M',w')$, so $M,w\models \SPal{A,\laa}\varphi$.

For soundness with respect to $(2)$ we use \emph{Minimum Rationality}:\\ $\PalPos{A,\laa}\varphi \iimplies \Bels{i}\SPalPos{A,\laa}\varphi$. Note that the equivalent first order frame condition to $(2)$ is $\forall y, z, (x \llbracket (A,\laa)_i \rrbracket y \Longrightarrow (\Rel{b}^i(x,z) \Longrightarrow z  \llbracket (A,\laa)_i \rrbracket^S y))$.

Assume $M,w \models \PalPos{A,\laa}\varphi$. By the semantics of $\PalPos{A,\laa}$, there exists $M',w'$ such that $(M,w)\llbracket (A,\laa)_i\rrbracket (M',w')$ and $M',w' \models \varphi$. From condition $(2)$, for all $(M,v)$, $\Rel{b}^i(w,v)$ implies $(M,v)\llbracket (A,\laa)_i \rrbracket^S (M',w')$. So, $M,w \models \Bels{i}\SPalPos{A,\laa}\varphi$.

\end{proof}

\begin{theorem}[Dynamic Extension Completeness]
	The dynamic extension of \DASL\ is complete with respect to \DASL\ models.
\end{theorem}
\begin{proof}
	We use the two Sahlqvist theorems SComT and SCorT for this proof. Both axiom schemas are clearly Sahlqvist implications, so we proceed by applying the Sahlqvist-van Benthem Algorithm to each. We abbreviate $(M,w)\llbracket (A,\laa)_i\rrbracket^S (M',w')$ with $\Rel{M,M'}^S(w,w')$ and $(M,w)\llbracket (A,\laa)_i\rrbracket (M',w')$ with $\Rel{M,M'}(w,w')$.
	
	$\Pal{A,\laa}p \iimplies \SPal{A,\laa}p$.
	\begin{align*}
	&\rightsquigarrow \forall P, y, (\Rel{M,M'}(x,y) \Longrightarrow P(y)) \Longrightarrow \forall z, (\Rel{M,M'}^S(x,z) \Longrightarrow P(z))\\
	&\rightsquigarrow \forall y, (\Rel{M,M'} (x,y) \Longrightarrow \lambda u.(\Rel{M,M'}(x,u))(y))\Longrightarrow \forall z, (\Rel{M,M'}^S (x,z) \Longrightarrow \lambda u. (\Rel{M,M'}(x, u))(z))\\
	&\rightsquigarrow \forall y, (\Rel{M,M'}(x,y)) \Longrightarrow \forall z, (\Rel{M,M'}^S(x,z) \Longrightarrow \Rel{M,M'}(x,z))\\
	&\equiv \forall y,z,  (\Rel{M,M'}(x,y)\Longrightarrow\Rel{M,M'}^S(x,z)\Longrightarrow\Rel{M,M'}(x,z))\\
	&\equiv \forall y,z, (\Rel{M,M'}(x,y)\cdot\Rel{M,M'}^S(x,z) \Longrightarrow \Rel{M,M'}(x,z))\\
	&\equiv \forall z, (\Rel{M,M'}^S(x,z)\Longrightarrow\Rel{M,M'}(x,z))\\
	&\equiv \Rel{M,M'}^S \subseteq \Rel{M,M'}
	\end{align*}
	
	$\PalPos{A,\laa}\tlnot p \iimplies \Bels{i}\SPalPos{A,\laa}\tlnot p$.
	\begin{align*}
		&\equiv \BPoss{i}\SPal{A,\laa}p \iimplies \Pal{A,\laa}p\\
		&\rightsquigarrow \forall P, (\exists y, (\Rel{b}^i(x,y) \cdot \forall z, (\Rel{M,M'}^S(y,z) \Longrightarrow P(z)))\Longrightarrow \forall z', (\Rel{M,M'}(x,z') \Longrightarrow P(z'))) \\
		&\rightsquigarrow \forall P,y, ((\Rel{b}^i(x,y) \cdot \forall z, (\Rel{M,M'}^S(y,z) \Longrightarrow P(z))) \Longrightarrow \forall z', (\Rel{M,M'}(x,z') \Longrightarrow P(z')))\\
		&\rightsquigarrow \forall y, (\Rel{b}^i(x,y) \Longrightarrow \forall z', (\Rel{M,M'}(x,z') \Longrightarrow \Rel{M,M'}^S(y,z')))\\
		&\equiv \forall y, z', (\Rel{M,M'}(x,z') \Longrightarrow (\Rel{b}^i(x,y) \Longrightarrow \Rel{M,M'}^S(y,z')))\\
		&\equiv \Rel{M,M'} \subseteq (\Rel{M,M'}^S \circ \Rel{b}^i)		
	\end{align*}
\end{proof}

Thus, for additional dynamic frame conditions  $\llbracket (A,\laa)_i \rrbracket^S \subseteq \llbracket (A,\laa)_i \rrbracket$ and 
$\llbracket (A,\laa)_i \rrbracket \subseteq (\llbracket (A,\laa)_i \rrbracket^S \circ \Rel{b}^i)$, \DASL\ is sound and complete. Next we turn to case studies in which we apply \DASL\ to the analysis of real aviation mishaps, to both prove its usefulness in modeling realistic human agents, and to show instances of an interesting contrapositive of the game theoretic inference from non-optimal action and rationality to a lack of knowledge.