...
The key to achieving this is through the composition of the doxastic relation with the epistemic relation. From a first person perspective, our model does collapse belief and knowledge together. That means that an agent cannot distinguish her mere beliefs from her genuine knowledge. 

\begin{theorem}[Weak Knowledge-Belief Indistinguishability]
	For all $w,v \in W$, if $w(\Rel{k}^i \circ \Rel{b}^i)v$ then for all $w' \in W$, $w\Rel{b}^iw'$ implies $\Rel{b}^i(w') = \Rel{k}^i(w') = \Rel{k}^i(v) = \Rel{b}^i(v)$.
\end{theorem}

\begin{theorem}[Strong Knowledge-Belief Indistinguishability]
