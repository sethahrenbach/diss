...
This section establishes the soundness and completeness of the logic. Often these properties are examined separately for a formal system, but some modal logics have features that allow the powerful Sahlqvist theorem to prove them both at once. Because the theorem is very powerful, and may be unfamiliar to some, we devote some space to explaining it and the conditions under which it applies. Without this groundwork, the proof of soundness and completeness will seem inadequate and suspicious.

We first present the features that make a modal logic formula a member of the class of Sahlqvist formulas. We then present the Sahlqvist theorem, which applies to the class of Sahlqvist formulas. Finally, we show that the axioms of DASL are Sahlqvist formulas, and therefore that the Sahlqvist theorem applies to them. This suffices to show soundness and completeness.
...
A \emph{Sahlqvist antecedent} is a formula built from $a)$ propositional letters prefixed by $n\geq0$ $\Box$'s and $b)$ negative formulas, $c)$ using only $\tlnot$, $\tland$, and $\Diamond$ to build the antecedent from $(a)$ and $(b)$ components.
...
\begin{theorem}[Sahlqvist's Theorem]~\label{thm:sahlqvist}
	Every Sahlqvist formula $\varphi$ has a corresponding first-order translation $\mathnormal{FO}(\varphi)$ and is valid for the frames for which $\mathnormal{FO}(\varphi)$ is canonical.
\end{theorem}

For soundness and completeness, it suffices to show that each DASL axiom is a Sahlqvist formula.
