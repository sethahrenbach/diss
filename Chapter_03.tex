...
We combine the above components to define what is is for a formula to be a Sahlqvist implication.
...
Fixpoint sahlqvist_formula (phi : formula) : Prop :=
	match phi with
		| FProp phi'=> True
		| FAnd phi1 phi2 => (sahlqvist_antecedent phi1) 
			/\ (sahlqvist_antecedent phi2)
		| FOr phi1 phi2 => not (sahlqvist_antecedent phi1) 
			/\ (positive_formula phi2)
		| FImp phi1 phi2 => (sahlqvist_antecedent phi1) 
			/\ (positive_formula (normal_form phi2))
...
For completeness, it suffices to show that each static \DASL\ axiom schema is a Sahlqvist formula. 
...
\begin{tcolorbox}
	...
	Example Lob_not_sahlqvist : forall (phi : prop) (a : DASL.Agents),
		not sahlqvist_formula (
			FK a (FK a (FProp phi) =f=> (FProp phi)) =f=> FK a (FProp phi)
		).
	Proof.
...
We prove that each of the modal axioms schemas of \DASL\ are Sahlqvist formulas using the above techniques, which can be seen in the appendix.
