...
The underlying logic of Coq is called the Calculus of Inductive Constructions, a dependently-typed constructive logic. One uses Coq by formalizing the target logic and its semantics in Coq and using what are called \emph{tactics} to manipulate proof objects. This chapter implements the previously described Dynamic Agent Safety Logic in Coq and formally models aviation mishaps, demonstrating the logic's ability to dynamically model safety-critical information flow in a real-world scenario. This will require translating the logic as it appears here and its metatheory into Coq, complete with tactics appropriate for the desired proofs and fully instantiated semantics, a process called \emph{mechanization}.
...
We use the following notation for implication.
\begin{tcolorbox}
	\begin{lstlisting}[language=Coq]
